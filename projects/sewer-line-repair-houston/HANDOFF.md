# Sewer Line Repair Houston Pros: Handoff
...
Vercel uses `npm run build` and serves `dist/`. The independent production project is live at `https://sewer-line-repair-houston.vercel.app/`. The apex and `www` custom domains are already assigned to this project and will become active after the domain is purchased and its Cloudflare DNS records point to Vercel.
