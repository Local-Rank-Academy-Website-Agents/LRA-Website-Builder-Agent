(() => {
  ...
  document.querySelectorAll(".nav-cta, .mobile-lead-cta").forEach((link) => { link.href = tel; link.textContent = `Call ${phone}`; });
  ...
  const footerNav = document.querySelector(".footer-top nav, .footer__intro nav"); if (footerNav && !footerNav.querySelector('[href^="tel:"]')) footerNav.insertAdjacentHTML("afterbegin", `<a href="${tel}">Call ${phone}</a>`);
  document.querySelectorAll("[data-lead-form]").forEach((form) => form.addEventListener("submit", async (event) => {
    ...
    catch (error) { console.error("Lead submission failed", error); if (status) { status.textContent = "We couldn’t send your request. Please call the number shown on this page."; status.classList.add("form-status--error", "is-visible"); } }
